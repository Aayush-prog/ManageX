...
const teamId = (req) => req.headers['x-active-team'] || null;

export const addExpense    = async (req, res) => { try { ok(res, await addExpenseService(req.body, req.user.id, teamId(req)), 201);                          } catch (e) { err(res, e); } };
export const getExpenses   = async (req, res) => { try { ok(res, await getExpensesService({ ...req.query, teamId: teamId(req) }));                           } catch (e) { err(res, e); } };
...
export const addBill       = async (req, res) => { try { ok(res, await addBillService(req.body, req.user.id, teamId(req)), 201);                              } catch (e) { err(res, e); } };
export const getBills      = async (req, res) => { try { ok(res, await getBillsService(teamId(req)));                                                         } catch (e) { err(res, e); } };
...
export const getSummary    = async (req, res) => {
  try {
    ...
    ok(res, await getSummaryService(month, startFrom, startTo, teamId(req)));
  } catch (e) { err(res, e); }
};
