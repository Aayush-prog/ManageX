...
const AddBillModal = ({ onClose, onCreated }) => {
  const [form,   setForm]   = useState({ vendorName: '', description: '', amount: '', dueDate: '' });
  const [saving, setSaving] = useState(false);
  const [error,  setError]  = useState('');
  const set = (k, v) => setForm((p) => ({ ...p, [k]: v }));
  ...
  const submit = async (e) => {
    ...
    setSaving(true); setError('');
    try {
      const payload = { ...form, amount: Number(form.amount), dueDate: form.dueDate || undefined };
...
const TABS = ['Expenses', 'Bills', 'Budgets', 'Summary'];
